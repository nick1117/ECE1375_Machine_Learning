...
for i in range(subsets):
    rand_nums_bin = np.random.permutation(X_train.shape[0]) 
    X_subset = X_train[rand_nums_bin[:1000]]
    y_subset = y_train[rand_nums_bin[:1000]]
...
KNN_error2 = error(X2,y2,KNN)
KNN_error1 = error(X1,y1,KNN)
KNN_error3 = error(X3,y3,KNN)
KNN_error4 = error(X4,y4,KNN)
KNN_error5 = error(X5,y5,KNN)
KNN_error_test = error(X_test,y_test,KNN)
...
LogReg_error3 = error(X3,y3,LogReg)
LogReg_error1 = error(X1,y1,LogReg)
LogReg_error2 = error(X2,y2,LogReg)
LogReg_error4 = error(X4,y4,LogReg)
LogReg_error5 = error(X5,y5,LogReg)
LogReg_error_test = error(X_test,y_test,LogReg)
...
DecTree_error4 = error(X4,y4,DecTree)
DecTree_error1 = error(X1,y1,DecTree)
DecTree_error2 = error(X2,y2,DecTree)
DecTree_error3 = error(X3,y3,DecTree)
DecTree_error5 = error(X5,y5,DecTree)
DecTree_error_test = error(X_test,y_test,DecTree)
...
rand_Forest.fit(X5,y5)

RandForest_Error5 = error(X5,y5,rand_Forest)
RandForest_Error1 = error(X1,y1,rand_Forest)
RandForest_Error2 = error(X2,y2,rand_Forest)
RandForest_Error3 = error(X3,y3,rand_Forest)
RandForest_Error4 = error(X4,y4,rand_Forest)
RandForest_Error_test = error(X_test,y_test,rand_Forest)
...
header = ['', 'SVM', 'KNN', 'LogReg', 'DecTree', 'RandForest']
...
print('errors for data with different methods')
print(table)
print(f'Voted Prediction Error: {voted_error}')